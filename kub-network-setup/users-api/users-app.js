...
app.post("/login", async (req, res) => {
  ...
  const response = await axios.get(
    `http://${process.env.AUTH_SERVICE_SERVICE_HOST}/token/` + //This env value AUTH_SERVICE_SERVICE_HOST comes from kubernetes, automatically gets service's IP with the given name running in the cluster
      hashedPassword +
      "/" +
      password
  );
